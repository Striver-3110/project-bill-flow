...
const StatsCard: React.FC<StatsCardProps> = ({
  ...
  return <div className={cn("bg-white rounded-lg border p-4 shadow-sm", styles.card, className)}>
      <div className="flex items-center">
        {Icon && <div className={cn("mr-4 rounded-full p-2", styles.icon)}>
            <Icon className="h-5 w-5" />
          </div>}
        <div>
          <p className="text-sm font-medium text-billflow-gray-500">{title}</p>
          <h3 className="text-2xl font-bold text-billflow-gray-900 mt-1 overflow-hidden">
            {value}
          </h3>
          {description && <p className="text-sm text-billflow-gray-500 mt-1">{description}</p>}
